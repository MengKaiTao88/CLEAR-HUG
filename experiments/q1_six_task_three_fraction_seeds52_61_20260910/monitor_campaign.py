#!/usr/bin/env python3
"""Read-only three-node progress monitor for the frozen three-fraction campaign."""
...
def inspect(node: tuple[str, str, str, int, str]) -> dict:
    ...
    command = (
        f"find {out}/100pct -mindepth 2 -maxdepth 2 -name train-val-complete.json -type f 2>/dev/null | wc -l; "
    ...
    return {
        ...
        "units_100pct": int(lines[0]), "units_10pct": int(lines[1]),
        "units_1pct": int(lines[2]),
        "hug_checkpoints": int(lines[3]), "hila_checkpoints": int(lines[4]),
        "hilar_checkpoints": int(lines[5]), "formal_tests": int(lines[6]),
        "gpu": lines[7], "model": model,
...
def main() -> None:
    ...
    result = {
        ...
        "totals": {
            ...
            "units_100pct": sum(x["units_100pct"] for x in healthy),
