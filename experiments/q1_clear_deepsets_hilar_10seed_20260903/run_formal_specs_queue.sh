...
OUT="${ROOT}/results/${CAMPAIGN}"; mkdir -p "${OUT}"
STATUS="${OUT}/${QUEUE}-formal-queue-status.json"; LOG="${OUT}/${QUEUE}-formal-queue.log"
exec > >(tee -a "${LOG}") 2>&1
write_status() {
...
for spec in "$@"; do
 ...
 bash "$(dirname "$0")/run_formal_task_seed.sh" "${spec%%:*}" "${spec##*:}"
done
write_status complete ""
echo "FORMAL_QUEUE_COMPLETE queue=${QUEUE} time=$(date -Is)"
