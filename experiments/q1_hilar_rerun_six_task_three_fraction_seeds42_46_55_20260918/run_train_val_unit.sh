...
EXPECTED=$(python - "${TOTAL}" "${FRACTION}" <<'PY'
import sys
...
HILAR_ROOT="${RUN}/hilar-training"; HILAR="${HILAR_ROOT}/parameter-matched-direct"
...
write_status(){ python - "${STATUS}" "$1" "${2:-}" "${FKEY}" "${TASK}" "${SEED}" <<'PY'
import json,os,sys
...
import hashlib,sys
want='15e456964c5f819aa882522a203946b515cc1034e217dfcbc474e5e50bf1719a'
got=hashlib.sha256(open(sys.argv[1],'rb').read()).hexdigest(); assert got==want,(got,want)
PY
...
y=np.load(root/'train_labels.npy',mmap_mode='r')
idx=np.arange(len(y),dtype=np.int64) if fraction==1.0 else np.asarray(random.Random(seed).sample(range(len(y)),int(len(y)*fraction)),dtype=np.int64)
...
python -m torch.distributed.run --nnodes=1 --nproc_per_node=1 --master_port="$((24000+SEED*10+ID))" "${ROOT}/mvp/run_fair_baseline_finetuning.py" \
 --dataset_dir "${QRS}" --output_dir "${HILA}" --log_dir "${HILA}-tb" --model CLEAR_MASKED_DEEPSETS_finetune_base \
 --finetune "${ROOT}/checkpoints/released_ckpt.pth" --trainable adapter --split_ratio "${FRACTION}" --sampling_method random \
 --weight_decay 0.05 --batch_size 256 --lr 5e-3 --update_freq 1 --warmup_epochs 10 --epochs 100 --layer_decay 0.9 \
 --save_ckpt_freq 100 --seed "${SEED}" --is_binary --nb_classes "${CLASSES}" --world_size 1 --atten_mask --cls_token_num 12 \
 --mask_ratio 0 --num_workers 10 --screen_only --screen_select_metric roc_auc --screen_patience 0
test -s "${HILA}/checkpoint-best.pth"
python - "${HILA}" "${TASK}" "${SEED}" "${FRACTION}" <<'PY'
import json,os,sys
from pathlib import Path
out,task,seed,fraction=Path(sys.argv[1]),sys.argv[2],int(sys.argv[3]),float(sys.argv[4])
rows=[json.loads(x) for x in (out/'log.txt').read_text().splitlines() if x.strip()]; assert len(rows)==100,len(rows)
b=max(rows,key=lambda x:float(x['val_roc_auc']))
p={'task':task,'seed':seed,'epochs':100,'train_fraction':fraction,'selection_metric':'macro_auroc','best_epoch':int(b['epoch']),'best_validation_auroc':float(b['val_roc_auc']),'best_validation_auprc':float(b['val_pr_auc']),'test_used_for_selection':False}
...
python "${CODE}/cache_paired_features.py" --root "${ROOT}" --dataset "${QRS}" --checkpoint "${HILA}/checkpoint-best.pth" \
 --output "${FEATURES}" --classes "${CLASSES}" --seed "${SEED}" --splits train val --train-split-ratio "${FRACTION}" --sampling-method random
stage=hilar-training; write_status running "${stage}"
python "${ROOT}/mvp/train_deepsets_anchored_residual.py" --features "${FEATURES}" --output "${HILAR_ROOT}" --task "${TASK}" \
 --classes "${CLASSES}" --seed "${SEED}" --batch-size 16 --selection-metric auroc --variants parameter-matched-direct
test -s "${HILAR}/checkpoint-best.pth"; test -s "${HILAR}/complete.json"
...
def sha(p):
 ...
 return h.hexdigest()
hila=run/'hila/checkpoint-best.pth'; hilar=run/'hilar-training/parameter-matched-direct/checkpoint-best.pth'
feature=json.loads((run/'features/feature-manifest.json').read_text()); residual=json.loads((run/'hilar-training/parameter-matched-direct/complete.json').read_text()); subset=json.loads((run/'subset-manifest.json').read_text())
