...
CAMPAIGN=q1-csn-low-label-10pct-20260909
...
"${ROOT}/.venv/bin/python" - "${INPUT}" "${SEED}" "${RUN}/subset-manifest.json" <<'PY'
import hashlib, json, random, sys
from pathlib import Path
import numpy as np
root, seed, output = Path(sys.argv[1]), int(sys.argv[2]), Path(sys.argv[3])
labels = np.load(root/'train_labels.npy', mmap_mode='r')
indices = np.asarray(
    random.Random(seed).sample(range(len(labels)), int(len(labels) * 0.1)),
    dtype=np.int64,
)
payload = {
    'seed': seed,
...
"${ROOT}/.venv/bin/python" -m torch.distributed.run --nnodes=1 --nproc_per_node=1 \
  ...
  --model CLEAR_HUG_finetune_base --trainable moe --split_ratio 0.1 \
...
"${ROOT}/.venv/bin/python" - "${HUG}" "${SEED}" <<'PY'
import json, os, sys
from pathlib import Path
...
payload={'task':'csn','seed':seed,'epochs':100,'train_fraction':0.1,
...
"${ROOT}/.venv/bin/python" -m torch.distributed.run --nnodes=1 --nproc_per_node=1 \
  ...
  --trainable adapter --split_ratio 0.1 --sampling_method random --weight_decay 0.05 \
...
"${ROOT}/.venv/bin/python" - "${HILA}" "${SEED}" <<'PY'
import json, os, sys
from pathlib import Path
out, seed = Path(sys.argv[1]), int(sys.argv[2])
rows=[json.loads(x) for x in (out/'log.txt').read_text().splitlines() if x.strip()]
assert len(rows)==100, len(rows)
best=max(rows,key=lambda row:float(row['val_roc_auc']))
payload={'task':'csn','seed':seed,'epochs':100,'split_ratio':0.1,'sampling_method':'random',
...
"${ROOT}/.venv/bin/python" "${CODE}/cache_low_label_features.py" \
  ...
  --train-split-ratio 0.1 --sampling-method random
...
stage=audit
write_status running "${stage}"
"${ROOT}/.venv/bin/python" - "${RUN}" "${SEED}" <<'PY'
...
assert hila_meta['seed']==seed and hila_meta['split_ratio']==0.1
assert features['checkpoint']==str(hila) and features['checkpoint_seed']==seed
assert features['train_split_ratio']==0.1 and features['splits']['train']['records']==1654
assert residual['selection_metric']=='auroc' and float(residual['anchor'])==0.0
payload={'schema_version':1,'status':'complete','task':'csn','seed':seed,'train_fraction':0.1,
 'sampling_method':'random','train_records':1654,'validation_is_full':True,'test_is_full':True,
