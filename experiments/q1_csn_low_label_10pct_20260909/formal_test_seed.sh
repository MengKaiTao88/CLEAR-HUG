...
r=Path(sys.argv[1]); paired=json.loads((r/'formal-test/paired/formal-test-result.json').read_text()); hug=json.loads((r/'formal-test/hug/formal-test-result.json').read_text())
payload={'status':'complete','task':'csn','seed':paired['seed'],'train_fraction':0.1,
