#!/usr/bin/env python3
"""Evaluate each frozen generic-control checkpoint on test exactly once."""
...
from protocol import (BASELINE_CAMPAIGN, CAMPAIGN, FORMAL_TEST_CAMPAIGN, METHODS,
                      REFERENCE_TEST_CAMPAIGN, SEEDS, TASKS, hilar_parameter_budget)
...
def metrics(labels: np.ndarray, probabilities: np.ndarray) -> dict:
    ...
    for index in range(labels.shape[1]):
        ...
        if np.unique(truth).size == 2:
            auroc = float(roc_auc_score(truth, probabilities[:, index])); aucs.append(auroc)
        auprc = float(average_precision_score(truth, probabilities[:, index])); aps.append(auprc)
        per_class.append({"class_index": index, "positives": int(truth.sum()),
                          "total": len(truth), "auroc": auroc, "auprc": auprc})
...
def evaluate_once(root: Path, task: str, seed: int, method: str,
                  device: torch.device) -> dict:
    output = (root / "results" / FORMAL_TEST_CAMPAIGN / "evaluations"
              / f"seed-{seed}" / task / method / "complete.json")
    if output.exists():
    ...
    baseline_checkpoint = torch.load(baseline_folder / "best.pt", map_location="cpu",
                                     weights_only=False)
    checkpoint_path = (root / "results" / CAMPAIGN / f"seed-{seed}" / task
                       / method / "best.pt")
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    if checkpoint["epoch"] != json.loads((checkpoint_path.parent / "complete.json").read_text(
            encoding="utf-8"))["best_epoch"]:
    ...
    if not np.array_equal(mean, np.asarray(baseline_checkpoint["mean"], dtype=np.float32)):
        raise RuntimeError("checkpoint normalization mean changed after validation selection")
    if not np.array_equal(std, np.asarray(baseline_checkpoint["std"], dtype=np.float32)):
        raise RuntimeError("checkpoint normalization std changed after validation selection")
    ...
    if abs(actual - target) / target > 0.01:
        raise RuntimeError("formal-test model no longer satisfies the locked parameter budget")
    ...
    result = {"state": "complete", "split": "test", "task": task, "seed": seed,
              "fraction": 1.0, "method": method, "metrics": metrics(labels, probabilities),
...
def summarize(root: Path) -> dict:
    rows = [json.loads((root / "results" / FORMAL_TEST_CAMPAIGN / "evaluations"
            / f"seed-{seed}" / task / method / "complete.json").read_text(encoding="utf-8"))
            for seed in SEEDS for task in TASKS for method in METHODS]
    ...
    for task in TASKS:
        ...
        for method in METHODS:
            ...
            by_task[task][method] = {metric: {
                ...
                "sd": float(np.std([row["metrics"][metric] for row in selected], ddof=1)),
                "values_by_seed": {str(row["seed"]): row["metrics"][metric] for row in selected}}
                for metric in metric_names}
    controls = {method: {metric: float(np.mean([row["metrics"][metric] for row in rows
                if row["method"] == method])) for metric in metric_names} for method in METHODS}
    reference = json.loads((root / "results" / REFERENCE_TEST_CAMPAIGN
                            / "metrics_summary.json").read_text(encoding="utf-8"))
    publication_table = {
        "tolerant_ecg_lp": reference["overall"]["tolerant_ecg"],
        "parameter_matched_mlp": controls["parameter-matched-mlp"],
        "generic_adapter": controls["generic-adapter"],
        "hilar": reference["overall"]["hila_k_lra"],
    }
    summary = {"state": "complete", "split": "test", "fraction": 1.0,
               "seeds": list(SEEDS), "rows": rows, "by_task": by_task,
               "controls_overall": controls, "publication_table": publication_table,
    ...
    atomic_json(root / "results" / FORMAL_TEST_CAMPAIGN / "metrics_summary.json", summary)
    return summary
...
def main() -> None:
    ...
    if args.summarize_only:
        print(json.dumps(summarize(root)["publication_table"], indent=2)); return
        ...
        for task in TASKS:
            for method in METHODS:
                if index % args.total_shards == args.shard:
                    print(json.dumps(evaluate_once(root, task, seed, method, device), indent=2),
                          flush=True)
                index += 1
