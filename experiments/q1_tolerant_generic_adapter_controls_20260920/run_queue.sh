...
GPU="${1:?usage: run_queue.sh GPU SHARD TOTAL_SHARDS}"
SHARD="${2:?usage: run_queue.sh GPU SHARD TOTAL_SHARDS}"
TOTAL="${3:?usage: run_queue.sh GPU SHARD TOTAL_SHARDS}"
index=0
for seed in "${SEEDS[@]}"; do
  for task in "${TASKS[@]}"; do
    for method in "${METHODS[@]}"; do
      if (( index % TOTAL == SHARD )); then
        "$PY" "$EXP/train.py" --root "$ROOT" --task "$task" --seed "$seed" \
          --method "$method" --device "cuda:$GPU"
      fi
      index=$((index + 1))
    done
  done
