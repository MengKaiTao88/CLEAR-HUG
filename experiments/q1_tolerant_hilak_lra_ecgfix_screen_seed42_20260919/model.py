"""Frozen HILA-K with a zero-initialized heartbeat-local residual branch."""
...
class HILAK(nn.Module):
    """Exact architecture used by the locked HILA-K checkpoint."""
...
class HILAKLRA(nn.Module):
    ...
    def forward(self, global_features: torch.Tensor, lead_features: torch.Tensor,
        ...
        if local_features.ndim != 3 or local_features.shape[-1] != EMBED_DIM:
            raise ValueError(f"invalid local feature shape: {local_features.shape}")
        if local_mask.shape != local_features.shape[:2]:
            raise ValueError(f"invalid local mask shape: {local_mask.shape}")
