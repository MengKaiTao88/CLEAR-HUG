...
case "${FKEY}" in 1pct) FRACTION=0.01 ;; 10pct) FRACTION=0.10 ;; *) exit 2 ;; esac
...
python - "${GATE}" <<'PY'
import json,sys
p=json.load(open(sys.argv[1])); assert p['status']=='passed' and p['formal_test_authorized'] is True and p['train_val_units']==120
PY
[[ -s "${RUN}/formal-test/complete.json" ]] && exit 0
mkdir -p "${RUN}/formal-test"
if [[ ! -s "${RUN}/features/test/labels.npy" ]]; then
 python "${CODE}/cache_low_label_features.py" --root "${ROOT}" --dataset "${QRS}" --checkpoint "${RUN}/hila/checkpoint-best.pth" --output "${RUN}/features" --classes "${CLASSES}" --seed "${SEED}" --splits test --train-split-ratio "${FRACTION}" --sampling-method random
fi
...
if [[ ! -s "${RUN}/formal-test/hug/formal-test-result.json" ]]; then
 python "${CODE}/evaluate_hug_formal.py" --root "${ROOT}" --dataset "${QRS}" --checkpoint "${RUN}/clear-hug/checkpoint-best.pth" --released-checkpoint "${ROOT}/checkpoints/released_ckpt.pth" --strict-labels "${RUN}/features/test/labels.npy" --output "${RUN}/formal-test/hug" --task "${TASK}" --seed "${SEED}" --classes "${CLASSES}"
fi
