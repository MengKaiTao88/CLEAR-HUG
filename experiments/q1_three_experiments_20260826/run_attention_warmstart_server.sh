...
OUT="${ROOT}/results/q1-three-experiments-20260827/attention-warmstart"
LOGDIR="${ROOT}/logs/q1-three-experiments-20260827"
mkdir -p "${OUT}" "${LOGDIR}"
exec > >(tee -a "${LOGDIR}/attention-warmstart.log") 2>&1
echo "started $(date -Is) host=$(hostname)" > "${LOGDIR}/attention-warmstart.status"
...
# These are the frozen development DeepSets baselines used by the original
# attention screen.  No formal-test arrays are read by the cache helper.
prepare_cache superdiagnostic 5 "${PTB_QRS}/superdiagnostic" \
  "${ROOT}/results/deepsets-anchored-residual-final/superdiagnostic-seed42/frozen-deepsets/checkpoint-best.pth" \
  "${PTB_QRS}/superdiagnostic"
prepare_cache rhythm 12 "${PTB_QRS}/rhythm" \
  "${ROOT}/results/formal-direct-residual-100/rhythm-seed42/frozen-deepsets/checkpoint-best.pth" \
  "${PTB_QRS}/rhythm"
prepare_cache cpsc2018 9 "${CPSC_QRS}" \
  "${ROOT}/results/formal-direct-residual-100/cpsc2018-seed42/frozen-deepsets/checkpoint-best.pth" \
...
for task_spec in \
  ...
  for seed in 42 43 44; do
    "${PY}" "${C}/train_set_transformer_warmstart_dev.py" \
...
echo "complete $(date -Is)" > "${LOGDIR}/attention-warmstart.status"
