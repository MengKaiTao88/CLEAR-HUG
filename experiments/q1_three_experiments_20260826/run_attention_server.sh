...
OUT="${ROOT}/results/q1-three-experiments-20260826/attention"
LOGDIR="${ROOT}/logs/q1-three-experiments-20260826"
mkdir -p "${OUT}" "${LOGDIR}"
exec > >(tee -a "${LOGDIR}/attention.log") 2>&1
echo "started $(date -Is)" > "${LOGDIR}/attention.status"
...
for task_spec in \
  ...
  for seed in 42 43 44; do
    "${PY}" "${C}/train_set_transformer_dev.py" \
...
echo "complete $(date -Is)" > "${LOGDIR}/attention.status"
