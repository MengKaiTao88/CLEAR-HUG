#!/usr/bin/env python3
"""Train locked HILA-K then LRA on an exact baseline low-label subset."""
...
from protocol import (BASELINE_CAMPAIGN, BATCH_SIZE, CAMPAIGN, FRACTIONS,
...
def baseline(root: Path, task: str, seed: int, fraction: float):
...
def train_hilak(root: Path, task: str, seed: int, fraction: float, device: torch.device) -> dict:
    output = root / "results" / CAMPAIGN / f"fraction-{fraction:g}" / f"seed-{seed}" / task / "hila-k"
    complete_path = output / "complete.json"
    if complete_path.is_file():
        complete = json.loads(complete_path.read_text(encoding="utf-8"))
        if complete.get("state") == "complete": return complete
    base_state, mean, std, base_result = baseline(root, task, seed, fraction)
    seed_everything(seed)
    full_train_set, val_set = datasets(root, task, mean, std)
    subset_indices = [int(value) for value in base_result["train_subset_indices"]]
    if len(subset_indices) != base_result["train_records"]:
        raise RuntimeError("baseline low-label subset size mismatch")
    train_set = Subset(full_train_set, subset_indices)
    loader = DataLoader(train_set, batch_size=BATCH_SIZE, shuffle=True,
                        generator=torch.Generator().manual_seed(seed), num_workers=0,
                        pin_memory=True, drop_last=False)
    model = HILAK(val_set.labels.shape[1], base_state).to(device)
    ...
    result = {"state": "complete", "stage": "hila-k", "task": task, "seed": seed,
              ...
              "train_records": len(train_set), "train_subset_indices": subset_indices,
...
def train_lra(root: Path, task: str, seed: int, fraction: float, device: torch.device) -> dict:
    output = root / "results" / CAMPAIGN / f"fraction-{fraction:g}" / f"seed-{seed}" / task / "hila-k-lra"
    complete_path = output / "complete.json"
    if complete_path.is_file():
        complete = json.loads(complete_path.read_text(encoding="utf-8"))
        if complete.get("state") == "complete": return complete
    base_state, mean, std, base_result = baseline(root, task, seed, fraction)
    hila_folder = root / "results" / CAMPAIGN / f"fraction-{fraction:g}" / f"seed-{seed}" / task / "hila-k"
    ...
    loader = DataLoader(train_set, batch_size=BATCH_SIZE, shuffle=True,
                        generator=torch.Generator().manual_seed(seed), num_workers=0,
                        pin_memory=True, drop_last=False)
    model = HILAKLRA(val_set.labels.shape[1], base_state, hila_checkpoint["state_dict"]).to(device)
    optimizer = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad],
                                  lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
...
def main() -> None:
    ...
    print(json.dumps(train_hilak(root, args.task, args.seed, args.fraction, device), indent=2))
    print(json.dumps(train_lra(root, args.task, args.seed, args.fraction, device), indent=2))
