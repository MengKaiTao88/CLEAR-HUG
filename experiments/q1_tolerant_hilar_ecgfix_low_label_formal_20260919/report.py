#!/usr/bin/env python3
"""Report validation metrics for the 1%/10% campaign."""
...
def checkpoint_folders(root: Path, task: str, seed: int, fraction: float) -> tuple[Path, Path]:
    base = root / "results" / CAMPAIGN / f"fraction-{fraction:g}" / f"seed-{seed}" / task
    return base / "hila-k", base / "hila-k-lra"


def evaluate(root: Path, task: str, seed: int, fraction: float, device: torch.device) -> dict:
    base_state, mean, std, _ = baseline(root, task, seed, fraction)
...
def main() -> None:
    ...
    rows = [evaluate(root, task, seed, fraction, device)
    ...
    for fraction in FRACTIONS:
      by_task = {}
      for task in TASKS:
        task_rows = [row for row in rows if row["task"] == task and row["fraction"] == fraction]
        by_task[task] = {}
      ...
      by_fraction[f"{fraction:g}"] = {"by_task": by_task, "overall": {
          method: {metric: float(np.mean([row["models"][method][metric] for row in rows
    ...
    result = {"state": "complete", "split": "validation", "seeds": list(SEEDS),
              "fractions": list(FRACTIONS), "threshold": 0.5,
              "f1_average": "macro", "accuracy": "exact-match/subset",
              "rows": rows, "by_fraction": by_fraction,
              "test_data_loaded": False, "test_evaluations": 0}
